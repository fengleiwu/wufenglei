...
@property (nonatomic, strong)UIButton *loaclB;
...
-(UIButton *)loaclB{
    if (!_loaclB) {
        _loaclB = [UIButton buttonWithType:UIButtonTypeSystem];
        _loaclB.frame = CGRectMake(0, 0, kScreenWidth/4, self.broadcastTypeV.height/2);
        [_loaclB setImage:[UIImage imageNamed:@"本地.png"] forState:UIControlStateNormal];
        _loaclB.imageEdgeInsets = UIEdgeInsetsMake(0,0,40,_loaclB.titleLabel.bounds.size.width);
        [_loaclB setTitle:@"本地台" forState:UIControlStateNormal];
        _loaclB.titleLabel.font = [UIFont systemFontOfSize:18];
        [_loaclB setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
        _loaclB.titleEdgeInsets = UIEdgeInsetsMake(50, _loaclB.titleLabel.bounds.size.width-80, 0, 0);
        _loaclB.tintColor = [UIColor orangeColor];
    }
    return _loaclB;
}
